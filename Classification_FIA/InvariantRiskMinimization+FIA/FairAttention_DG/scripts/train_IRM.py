...
def validation(model, criterion, optimizer, validation_dataset_loader, epoch, result_dir=None, dataset_info=None, num_classes=2):
    ...
    with torch.no_grad():
        for i, (input1, input, target, attr) in enumerate(validation_dataset_loader):
...
#     val_files = list(datasplit['val_files'])
    ...
    train_havo_dataset = Harvard_DR_Fairness(args.data_dir, train_files, modality_type=args.modality_types, task=args.task, resolution=args.image_size, attribute_type=args.attribute_type, needBalance=args.need_balance, dataset_proportion=args.dataset_proportion)
    val_havo_dataset = Harvard_DR_Fairness(args.data_dir, test_files, modality_type=args.modality_types, task=args.task, resolution=args.image_size, attribute_type=args.attribute_type)
    test_havo_dataset = Harvard_DR_Fairness(args.data_dir, test_files, modality_type=args.modality_types, task=args.task, resolution=args.image_size, attribute_type=args.attribute_type)
