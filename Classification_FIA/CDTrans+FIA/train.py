...
if __name__ == '__main__':
    ...
    folder = 'FairDomain_Classify'

    testlist = np.loadtxt('FairDomain_Classify/test.txt', dtype=str)
    ...
    for i, f in enumerate(testlist):
        ...
        octfname = 'oct_'+f.split('.')[0]+'.jpg'
        imgattr[octfname] = attr
        
    trainlist = np.loadtxt('data/FairDomain_Classify/train.txt', dtype=str)
    ...
    if cfg.MODEL.UDA_STAGE == 'UDA':
        ...
        scheduler,  
        loss_func,
        num_query, args.local_rank, imgattr
    )
    else:
