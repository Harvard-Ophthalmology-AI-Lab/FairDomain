...
class build_uda_transformer(nn.Module):
    ...
    def forward(self, x, x2, label=None, cam_label= None, view_label=None, domain_norm=False, attrs=None, attr_flag=False, return_logits=False, return_feat_prob=False, cls_embed_specific=False):  # label is unused if self.cos_layer == 'no'
        inference_flag = not self.training
        global_feat, global_feat2, global_feat3, cross_attn = self.base(x, x2, cam_label=cam_label, view_label=view_label, attrs=attrs, attr_flag=attr_flag, domain_norm=domain_norm, cls_embed_specific=cls_embed_specific, inference_target_only=inference_flag)
