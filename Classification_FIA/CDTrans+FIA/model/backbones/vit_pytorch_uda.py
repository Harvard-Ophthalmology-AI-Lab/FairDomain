...
class Cond_Encoder(nn.Module):
    ...
    def forward(self, input, attr):
#         torch._assert(input.dim() == 3, f"Expected (batch_size, seq_length, hidden_dim) got {input.shape}")
        print('555555555555555555', input.shape)
...
class Block_3_branches(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.condition_embedding = nn.Embedding(2, mlp_hidden_dim)
        self.cond_encoder = Cond_Encoder(32, 1, 1, 768, 768, 0, 0, norm_layer)
        
    def forward(self, x, x2, x1_x2_fusion, use_cross=False, use_attn=True, domain_norm=False,inference_target_only=False, attrs=None, attr_flag=False):
        if inference_target_only:
            ...
            xa_attn, xa_attn2, xa_attn3, cross_attn = self.attn(self.norm1(x),self.norm1(x2), inference_target_only=inference_target_only)
            ...
            # add fairattention
            if attr_flag:
                condition_embed = self.condition_embedding(attrs)
                condition_embed = condition_embed.unsqueeze(1).repeat(1, xa.size(1), 1)
                xa = self.cond_encoder(xa, condition_embed)
                xb = self.cond_encoder(xb, condition_embed)
                xab = self.cond_encoder(xab, condition_embed)
...
class ConvStemEmbed_stride(nn.Module):
    """ Image to Patch Embedding
    """
    def __init__(self, img_size=224, patch_size=16, stride_size=20, in_chans=3, conv_channel= [24, 48, 96, 192], embed_dim=768):
        super().__init__()
        img_size = to_2tuple(img_size)
        patch_size = to_2tuple(patch_size)
        stride_size_tuple = to_2tuple(stride_size)
...
class TransReID(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm,local_feature=False, aie_xishu =1.0, use_cross=False, 
                 use_attn=True, block_pattern='normal'):
        super().__init__()
    ...
    def forward_features(self, x, x2, camera_id, view_id, domain_norm=False, cls_embed_specific=False,inference_target_only=False,
                        attrs=None, attr_flag=False):
        B = x.shape[0]
        # print('x size is {} x2 size is {}'.format(x.size(), x2.size()))
        ...
        if self.local_feature:
            ...
            if self.block_pattern == '3_branches':
                ...
                for i, blk in enumerate(self.blocks):
                    x,x2,x1_x2_fusion, cross_attn = blk(x,x2,x1_x2_fusion,use_cross=self.use_cross,use_attn=self.use_attn, domain_norm=domain_norm, inference_target_only=inference_target_only, attrs=attrs, attr_flag=attr_flag)
    ...
    def forward(self, x, x2, cam_label=None, view_label=None, domain_norm=False, cls_embed_specific=False,inference_target_only=False, attrs=None, attr_flag=True):
        x = self.forward_features(x, x2, cam_label, view_label, domain_norm, cls_embed_specific,inference_target_only, attrs, attr_flag)
        return x
...
def uda_vit_small_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_path_rate=0.1, drop_rate=0.0, attn_drop_rate=0.0, local_feature=False, aie_xishu=1.5, **kwargs):
    model = TransReID(
...
def uda_vit_base_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_path_rate=0.1, local_feature=False,aie_xishu=1.5,use_cross=False, use_attn=True, **kwargs):
    model = TransReID(
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6),  aie_xishu=aie_xishu, local_feature=local_feature, use_cross=use_cross, use_attn=use_attn, **kwargs)
