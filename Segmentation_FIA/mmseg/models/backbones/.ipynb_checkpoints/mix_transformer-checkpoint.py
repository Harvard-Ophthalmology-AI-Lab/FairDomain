...
class Fair_Encoder(nn.Module):
    ...
    def __init__(
        ...
        self.condition_embedding = nn.Embedding(3, hidden_dim)
    ...
    def forward(self, input, attr):
        ...
        input = input.reshape(n, c, h*w).permute(0, 2, 1)  # 2, 256, 512
        
        
        attr = self.condition_embedding(attr)
        attr = attr.unsqueeze(1).repeat(1, input.size(1), 1)  # [批次大小, 序列长度, 条件维度]
...
class MixVisionTransformer(BaseModule):
    ...
    def __init__(self,
        ...
        self.fair_attention1 = Fair_Encoder(seq_length=128*128, num_layers=1, num_heads=num_heads[0], hidden_dim=embed_dims[0], mlp_dim=embed_dims[0]*2, dropout=0.0, attention_dropout=0.0)
        self.fair_attention2 = Fair_Encoder(seq_length=64*64, num_layers=1, num_heads=num_heads[1], hidden_dim=embed_dims[1], mlp_dim=embed_dims[1]*2, dropout=0.0, attention_dropout=0.0)
        self.fair_attention3 = Fair_Encoder(seq_length=32*32, num_layers=1, num_heads=num_heads[2], hidden_dim=embed_dims[2], mlp_dim=embed_dims[2]*2, dropout=0.0, attention_dropout=0.0)
        self.fair_attention4 = Fair_Encoder(seq_length=16*16, num_layers=1, num_heads=num_heads[3], hidden_dim=embed_dims[3], mlp_dim=embed_dims[3]*2, dropout=0.0, attention_dropout=0.0)
...
#             print("---------------")
    ...
    def forward_features(self, x, attr_label):
        B = x.shape[0]
        outs = []
        # pdb.set_trace()
        # stage 1
        ...
        if attr_label is not None:
            x = self.fair_attention1(x, attr_label)
        ...
        if attr_label is not None:
            x = self.fair_attention2(x, attr_label)
        ...
        if attr_label is not None:
            x = self.fair_attention3(x, attr_label)
        ...
        if attr_label is not None:
            x = self.fair_attention4(x, attr_label)
            
        outs.append(x)

        return outs

    def forward(self, x, attr_label):
        x = self.forward_features(x, attr_label)
        # x = self.head(x)
