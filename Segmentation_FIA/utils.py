...
def test_single_image(image, label, model, interp, classes, multimask_output, patch_size=[256, 256], input_size=[224, 224],
    ...
    with torch.no_grad():
        ...
        output_masks = interp(model(inputs))
...
def test_single_image_dg(image, label, model, interp, classes, multimask_output, patch_size=[256, 256], input_size=[224, 224],
                       test_save_path=None, case=None, attr_label=0, idx=None):
    label = label.squeeze(0).cpu().detach().numpy()
