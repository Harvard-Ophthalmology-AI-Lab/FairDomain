...
class RandomGenerator(object):
    ...
    def __call__(self, sample):
        ...
        # print("2------", np.unique(sample['label']))
...
class FairDomain_dataset(Dataset):
    def __init__(self, base_dir, split, list_dir, balanced=False, bal_attr='race', \
    ...
    def __getitem__(self, idx):
        ...
        if(self.img_type == "fundus_slo"): # target
            image, label = data["fundus_slo"], data['slo_disc_cup']
        
        elif(self.img_type == "fundus_oct"): # source
            image, label = data["fundus_oct"], data['oct_disc_cup']
        ...
        # print("before ----", np.unique(sample['label']))
        if self.transform:
            sample = self.transform(sample)
        # print("after ----", np.unique(sample['label']))
        # return sample
        return sample['image'], sample['label'], sample['attr_label'], data_path
